...
try {
  ...
  // the document claims who generated it and who confirmed it, and an
  // import rules on nothing (design doc 0009 §3.2) — so the claim is kept
  // beside the concept as `received` (0046 §2.2) and the trust tier stays
  // unverified. Both halves are asserted, because a line that drew the
  // claim *as* the ledger would be the bug worth catching.
  //
  // insights/reading-revenue rather than metrics/revenue: the setup rules
  // on that one deliberately, so the review page has a trend to draw, and
  // its chip would make the second check pass for the wrong reason. This
  // one is also claimed by two different people — generated by sato,
  // confirmed by tanaka — so the grouping is exercised as well as the
  // claim.
  //
  // Asserted on the person the bundle names, who appears nowhere else on
  // the page: the ledger's own line names whoever ran the import.
  await go('#/k/insights/reading-revenue');
  ...
  await check('and the claim did not move the trust tier',
    await waitFor(`${textOf('#view')}.includes('売上の読み方')
      && !document.querySelector('#view .verified-mark')`), shown);
