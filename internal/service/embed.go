...
func (s *Service) Reembed(ctx context.Context, cursor string, limit int) (*ReembedResult, error) {
	...
	if s.Embedder == nil {
		return nil, Unsupportedf("semantic search is not enabled on this deployment: it is the default on Google Cloud once the service identity may call Vertex AI (design doc 0147 §1)")
	}
