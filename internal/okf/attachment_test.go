...
func TestFromBundleNamespaceHierarchicalID(t *testing.T) {
	...
	_, atts, skipped := FromBundle(files)
	if len(atts) != 1 || atts[0].ID != "query/sales/monthly" || atts[0].Name != "expected.txt" {
		t.Fatalf("atts = %+v", atts)
	}
	if len(skipped) != 0 {
		t.Errorf("skipped = %v, want none", skipped)
...
func TestAttachmentPath(t *testing.T) {
	...
	if p := AttachmentPath("insight/sales/revenue-reading", native); p != "insight/sales/revenue-reading/weekly.png" {
		t.Errorf("canonical path = %q", p)
	}
	foreign := &domain.Attachment{Name: "er.png", OKFPath: "diagrams/er.png"}
	if p := AttachmentPath("table/orders", foreign); p != "diagrams/er.png" {
