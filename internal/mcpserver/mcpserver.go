...
type searchIn struct {
	...
	FM       map[string]string `json:"fm,omitempty" jsonschema:"filter by any frontmatter key, exactly: {\"owner\": \"finance\"} finds entries whose frontmatter says owner is finance, matching a scalar or a member of a list. Every pair must match. It carries the keys this tool does not name — a producer's own, or one a later OKF version added — and only those: type, status, tags, sources and stale_after are refused here, because this tool reads them through fields of its own that answer a different question (a document that says no status reads as stable to statuses and as nothing to fm). Use types, statuses, tags, source and sort=stale_after for those"`
