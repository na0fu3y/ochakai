...
主張ではなく公開された実測である。五つ挙げる。**どれも ochakai を測った
ものではない**(最初の四つは 2026-08-21、五つ目は 2026-09-22 に原典を開いて
確かめ、採取時点の数字のまま置いてある)。
...
- **置き方と門を切り分けた実験。** EvoOntology(中国人民大学、
  [arXiv:2609.15779](https://arxiv.org/abs/2609.15779)、2026-09)は
  データエージェントの前に置く意味層を MCP サーバーにし、エージェントの
  軌跡から型付きの編集を提案して、検証セットで親と比べて上がったものだけを
  採る。この枠に当たる結果が三つある。**同じ中身をプロンプトに静的に前置
  すると、層が無いときをむしろ下回ることがあり**(DDR-Bench の
  Trajectory-Wise で Claude-Sonnet-5 が −15.0)、ツールで引かせると六つの
  バックボーンすべてで上がった(平均 +17.8)。**育てるループから門 —
  親と候補の対比較 — を外すと最も大きく落ち**(−11.2)、次に効いたのは
  失敗を中身の問題か見せ方の問題かに振り分ける段だった(外すと −6.3)。
  中身で最も効いたのは語を列に結ぶ対応づけと、観測した値の分布のような
  根拠の記録で(外すと −13.4 と −8.7)、定義の散文だけではなかった。
  著者自身が自分の実装を測った数字で、ここでは再現していない。

最初の四つが揃って言っているのは一つのことである — 精度を運んでいるのは
モデルではなく、人が書いたコンテキストのほうである。**そしてどれも、それを
どこに置くべきかは言っていない。** LookML でも semantic model でも wiki でも
同じ数字は出るはずで、上の四つはどれもその形で持っている。
...
セットの点数だけだが、ochakai が配るのは人が確かめたことそのものなので、
比較が通っても検証の印は付けない。次に、**ツールの層は育てない**。あちらの
改善の 57% はエージェントに見せる関数と説明を書き換える層から来ており、
これは差し出しているものである — ochakai の MCP はすべてのクライアントに
同じ一つの契約で配られるので、デプロイごとには動かさない。デプロイごとに
動かせる見せ方(`synonyms` と説明)だけがループに入る。最後に、あちらは
バックボーンごとに別の層を育て、他のバックボーンに移すとどの組でも 6.6
以上落ちると測っている。ochakai は一つのベースをすべてのクライアントに
...
- **検索。** 埋め込みを既定でオンにした hybrid search
  ([0147](design/0147-search-and-the-default-a-base-was-made-with.md) §1)、住所で
...
- **Google Cloud 推奨、外は認証だけ。** C2 の secret-zero という性質は
  ...
  **ただし「lexical のみ」がどれだけ落ちるかは、推測ではなく測ってある。**
  ...
  ([0147](design/0147-search-and-the-default-a-base-was-made-with.md) §1)から、
