...
後者は人の裁定だが、比べる一手は利用者の手順に置いてある
([最初のひと月](guides/onboarding.md) §3)。**採らないものも二つある。**
あちらは層を作るのも育てるのも LLM のエージェントで、門は検証セットの点数
だけである — ochakai が中に LLM を置かず、人の裁定を門にするのは
([0081](design/0081-what-ochakai-is-and-what-it-refuses-to-hold.md))、
人が確かめたことそのものを配るためである。そしてあちらはバックボーンごとに
別の層を育て、他のバックボーンに移すとどの組でも 6.6 以上落ちると測って
いる。ochakai は一つのベースを一つの契約ですべてのクライアントに配るので、
特定のモデルに合わせて育てる道は取らない — 合わせた分は、次に繋いだ
クライアントが払う。
