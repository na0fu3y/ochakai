...
Flags:
  ...
  -fm key=value
    	filter by a frontmatter key=value, exactly (repeatable, AND-ed) — for keys with no flag of their own, a producer's or a later OKF version's; a value spelling a number or a boolean matches the typed one too (--fm required=true)
  -json
    	print the raw JSON response
  -limit int
