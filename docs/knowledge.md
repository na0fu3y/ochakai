...
the body — and its **id is its address**: the concept lives in the bundle at
its id plus `.md`, which is the same place an export writes it and the same
place `ochakai get` reads it from. Reading a concept, editing it, and sending
...
of an exported bundle comes from concept IDs alone (design doc
...
Concepts relate to each other through **the markdown links in their body** —
there is no links field to fill in. Write `[revenue](/metrics/revenue.md)` in
the prose and the concept links to `metrics/revenue`; the target gains a
...
`sources` is the material a concept derives from — give one an `id` and a
...
the two signals stay independent. A draft somebody checked and a stable concept
...
`rejected_by` / `rejected_at` beside the concept's real status, and import
...
the computation belong to whoever consumes the concept (SPEC §5.1, §10.5).
...
an all-hiragana window is grammar and matches nearly everything. Concepts are
then ranked by how much of the *rare* part of the query they contain.

That finds the right concepts for a keyword and for a Japanese question, but
it is still a bag of words: ask an English question and a concept sharing
...
answered by scanning the table: about 16 ms per search across 5000 concepts,
