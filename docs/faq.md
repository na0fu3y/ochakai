...
(設計ドキュメント
[0146](design/0146-search-and-the-default-a-base-was-made-with.md))— がそれを断り、
...
からも Web UI からも何でも編集できる。これは面の規則である: MCP には、
安全な置き換えを表現できる `If-Match` の precondition を運ぶ道が無い(設計
...
ことになるからである。REST と CLI ではその復活が許される —
キュレーションの面だからである。**却下された id はこれに当たらない**:
却下は削除であり、その墓標は普通の墓標なので、MCP からも書き直せる
(設計ドキュメント 0135 §3)。

### エージェントは検証できるか

**MCP からはできない。REST と CLI からはできて、その検証は
machine-confirmed で止まる。** `POST /api/v1/review/{id}` は打った
...
**人に代わって打たれた検証は、その人の検証である。**
`Ochakai-On-Behalf-Of: human:<x>` を載せた呼び出しは、台帳に
`human:<x> via process:<呼び出し元>` と残り、その行は `human:` の行
として数えられる — 同梱の Web UI(`serve-ui`)の検証ボタンがこの形
そのものである。読める範囲・書ける範囲も、エージェントの turn の持ち主も、
呼び出し元ではなく `<x>` のものになる。だからこの信頼を支えているのは
[`OCHAKAI_DELEGATING_CALLERS`](configuration.md) の一覧で、そこに載せた
呼び出し元は、誰の名でも human-reviewed を付けられる。載せるのは、本人の
サインインを自分で確かめるアプリだけにする(設計ドキュメント
[0065](design/0065-identity-and-provenance.md) §3)。
...
だから**エージェントが主たる書き手になっても、面は一つも変わらない**。
変わるのは human-reviewed の段が空になることで、それは台帳が正直に
そう言っている状態である。ochakai が代わりに保証することはない — 誰が
確かめたかを記録し、判断は参照側が provenance を見て行う(設計
ドキュメント [0065](design/0065-identity-and-provenance.md) §1)。
自分の書いた draft を自分で検証したエージェントも同じで、台帳には
`created_by` と検証者の両方が残るので、**同じ actor かどうかは読めば
分かる** — 読むのは、それを信じるかを決める側である。
