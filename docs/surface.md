...
**5,700 → 5,800 は dogfooding の玄関である(DOC 24 → 25)。**
ochakai 自身の開発・運用ナレッジが [kb/](../kb) の OKF バンドルに
なり、ローカルインスタンスで想起と書き戻しのループを回し始めた。
ナレッジそのものは数えない — frontmatter を持つ md は ochakai が
保存するものであって説明ではない、という下の箇条は `kb/bundle` にも
そのまま当たる。数えるのは、人がループを立ち上げるために読む一枚
[kb/README.md](../kb/README.md) だけで、区切りをまたいだのはその
一枚ぶんである。
