...
**5,700 → 5,800 は、並行 PR がこの一行で行き違ったぶんである。**
[#553](https://github.com/na0fu3y/ochakai/issues/553) が kb/ の運用を
書いたページで `DOC` を 24 → 25 にしたとき天井は据え置かれ、ほぼ同時に
着地した別の PR のマージが天井を 5,800 から 5,700 へ戻していたので、
main が 5,708 で落ちた。**100 行の区切りが減らしたのは衝突の頻度で
あって、二本が同じ区切りをまたぐときの行き違いではない** — 上の段落が
「上下どちらの向きにも同じ幅で効く」と書いた仕組みの、その隙間である。
ページが一枚増えたことは決定として正しいので、天井をそれに合わせる。
