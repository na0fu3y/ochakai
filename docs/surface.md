...
予算が返らないまま残った。数行の編集がそのたびに CI を落とすのは
やり過ぎだが、28 行の未返却は見逃していい余白ではない、というのが
`DOC-LINES-SLACK: 10` の意味である。
...
見込まれていた。日本語は語の間に空白を置かないので一行が多くを運ぶ、
という理屈だったが、全角文字は同じ 70 桁に半分しか入らない。実測は
...
5,032 → 5,050 は C8 の四枚目、
[mcp-clients.md](guides/mcp-clients.md) を日本語にした分である
([#444](https://github.com/na0fu3y/ochakai/issues/444))。ページ自身は
365 → 383 行、18 行増えている — #398 の loop.md が 114 → 120 行で見せた
「翻訳はこの天井に対してほぼ中立、わずかに増える」という実測を、四枚目
...
残る 18 行はこの段落自身の分で、DOC のほかの段落と同じくここに数えて
いる。5,050 → 5,068。
