...
**5,600 → 5,700 は、到達経路ですらない。** README は公開デモに三行で
繋いで `ochakai context` を打たせておきながら、**その出力を一度も見せて
いなかった** — 評価している人が「で、何が返るのか」を知るには、まず
`go install` する必要があった。足したのはその答えで、デモバンドルの実物
から取った抜粋である(季節性の表、閾値、そして draft が未決の三点を
抱えたまま返ること)。**九つの次元は一つも動いていない**、というのが
この行の要点である: 「使っている姿が見えない」への答えとして最初に出て
きた案は Web UI に LLM を載せることで、それは 0081 §6 と
[0067](design/0067-four-faces-and-what-they-decline.md) §5.4 が断って
いる当のものだった。**面を足さずに散文で払えるなら、そちらが安い** —
この文書が数えているものの中で、DOC-LINES はいちばん安い通貨である。
