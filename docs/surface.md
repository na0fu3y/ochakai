...
**5,700 → 5,800 は、この仕組みが初めて main を赤にしたまたぎである。**
0082 の #554 と kb/ の #553 は、どちらも自分のベースに対して天井の内側で
着地しており、CI は両方に緑を返した。マージ順に積むと 5,708 行で、
**誰も越えると思っていない境界を二本の PR が合流して越えた。**
5,200 → 5,300 で「別々の PR の散文が合流する」ことは既に一度書いて
いるが、あのときは片方が後追いで気づいて鳴らせた。今回そうならなかった
のは、**天井の検査が PR 単位であり、マージ後のベースに対して回り直す
関門が無い**からである。これは仕組みの穴ではなく値段で、一つのファイルの
一行を読むだけの検査に払える範囲を超えている — 直し方は merge queue を
足すことではなく、**赤くなった main を次の PR が直すこと**である。
それがこの段落で、着地は 5,721 行、`DOC-LINES-SLACK` の内側に 79 行の
余白を残している。
