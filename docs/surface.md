...
[golden-query-canary.md](guides/golden-query-canary.md) を日本語にした
分である([#441](https://github.com/na0fu3y/ochakai/issues/441))。ページ
自身は 159 → 163 行、4 行しか増えていない — CI の YAML スニペットと
shell コマンドはコメント二箇所を除けば一文字も動かないが、issue が
言う「page の半分ほど」に対してフェンス行を数えると実際には四分の一
強で、大きく見積もっていた。地の文だけのページ(loop.md は 114 →
120、configuration.md は 75 → 84)より増分が小さいのは、訳す対象が
そもそも少なかったからで、issue の見積もりの粗さより行の実数のほうが
天井には効く。被リンクは README・compatibility・architecture の三本
あったが、`(Japanese)` はどれも既存の折り返しの中に収まり、被リンク側
の行数は一行も動かなかった — configuration.md のときは被リンク九本が
7 行を押し出したのと対照的である。この段落を含めて 5,032 → 5,050。
