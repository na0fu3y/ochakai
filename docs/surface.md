...
**6,000 → 6,100 は姿勢が一つ増えたぶんである
([0087](design/0087-a-sandbox-says-it-is-one.md))。** ページは増えて
いない — 増えたのは運用ガイドの一節(復元ジョブの手順)と、設定・英語
ページの各一段落である。姿勢は利用者が**選ぶ**ものなので、綴りだけを
`OCHAKAI_MODE` の行に足して手順をどこにも書かなければ、選べない選択肢に
なる。
