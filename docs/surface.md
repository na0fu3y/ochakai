...
開発・運用ナレッジが [kb/](../kb) の OKF バンドルになった。ナレッジ
そのものは数えない — frontmatter を持つ md は保存されるものであって
説明ではない、という下の箇条は `kb/bundle` にもそのまま当たる。数える
のは、人がループを立ち上げるために読む一枚
[kb/README.md](../kb/README.md) だけである。
