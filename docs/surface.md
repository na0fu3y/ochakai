...
**幅は 100 行、つまり天井は実数を切り上げた百の位である。** 当初は 10
行にしていたが、それだと**マニュアルを触るすべての PR がこの一行を
書き換える** — 並行して進む翻訳が毎回ここで衝突し、しかも 6 行の変更が
天井を動かすなら「上限を動かす PR は声に出して言っている」という上の
段落が意味を失う。100 行をまたぐのは実際に大きくなった瞬間だけで、
そのときの一段落は読む価値がある。上下どちらの向きにも同じ幅で効く。
...
5,032 → 5,046 は C8 の四枚目、
[deploy/terraform/README.md](../deploy/terraform/README.md) を日本語に
した分である([#455](https://github.com/na0fu3y/ochakai/issues/455))。
ページ自身は 182 → 186 行、4 行増えている。README にはこのページへの
リンクが二本あり、どちらにも `(Japanese)` を足す必要があったが、片方は
もともと二行に折り返していた行の書き直しで吸収され、正味の増分は 1 行
だけだった。残る 9 行はこの段落自身の分で、DOC のほかの段落と
同じくここに数えている。
...
5,046 → 5,068 は C8 の五枚目、
[docs/guides/mcp-clients.md](guides/mcp-clients.md) を日本語にした分である
([#454](https://github.com/na0fu3y/ochakai/pull/454))。ページ自身は
365 → 383 行、18 行増えている — #398 の loop.md が 114 → 120 行で見せた
「翻訳はこの天井に対してほぼ中立、わずかに増える」という実測を、五枚目
でも裏付けている。クライアント名以外の見出しを日本語にした二箇所、
`#what-the-bridge-needs` と
`#clients-that-cannot-reach-your-deployment` は、外部の deploy ガイドと
README がアンカー付きで指しているので、見出しとは別に `<a id>` を
四行分置いて元のアンカーを残した。このページを指す被リンクは README・
ROADMAP・deploy/cloudrun のガイド・troubleshooting の四文書、六箇所に
あり、そのどれにも `(Japanese)` を足したが、どの一文もちょうど元の
改行に収まって一行も外へ出さなかった — #438 の configuration.md が同じ
作業で 7 行を文書の外に出したのとは対照的で、被リンクの代金が漏れる
かどうかは翻訳の中身ではなく、リンクを含む段落の余白次第だと分かる。
残る 22 行はこの段落自身の分で、DOC のほかの段落と同じくここに数えて
いる。5,046 → 5,068。
