...
消し、理由を裁定として残す(設計ドキュメント 0135)。理由は履歴と
...
**検証は編集で失効しない。** OKF は `verified` を内容の変更から独立させて
いる(SPEC §5.2)ので、検証済みの concept を編集しても ✓ は消えず、
四つ目のキューにも入らない。代わりに**そう言う**: 詳細ページでは
「検証後に編集」が ✓ の隣に付き、`ochakai get` の provenance 行が
