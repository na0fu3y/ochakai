...
なく**投影**で埋まる。`ochakai seed` にスキーマを渡すか、ジョブ履歴から
起こすかのどちらかで、どちらも ochakai が倉庫に触らない形をしている —
クエリを撃つのはあなたの identity であり、**どちらも全件 draft で着地
する**。投影されたスキーマは骨格であって、ナレッジになるのは、そのカラム
が何を意味するかを誰かが言った瞬間である。
