...
| `OCHAKAI_OAUTH_CLIENT_ID` | エージェントが提案した SQL を、チームの Web UI(`serve-ui`)が**本人の身元で**実行できるようにする Google OAuth の Web クライアント ID(`<number>-<id>.apps.googleusercontent.com`、設計ドキュメント [0142](design/0142-ochakai-carries-a-data-agent-that-does-not-rule.md) §4)。**secret ではない** — 公開の識別子で、`stats` がページに渡し、ページが Google のサインインをポップアップで開いて `bigquery.readonly` のトークンを受け取り、BigQuery を直接呼ぶ。トークンはページのメモリにだけあり、サーバーには届かない。実行するのは、提案を読んだ人が「実行」を押したとき、またはその人が会話ごとに一度「以後の SQL も確かめずに実行する」に同意したあとで(同意は会話が終われば消え、人が何も書かずに続けて走るのは 6 本まで)、一回の課金上限は 10 GiB。Google Cloud のコンソールで種類「ウェブアプリケーション」のクライアントを作り、**承認済みの JavaScript 生成元**に serve-ui のオリジン、**承認済みのリダイレクト URI** に `<オリジン>/oauth.html` を登録する。手元の `ochakai ui` はこれを使わない — ポップアップを出さず、プロキシが実行する(`OCHAKAI_AGENT` の行)。`OCHAKAI_AGENT` 無しでは起動を断る。クライアントの作り方を含む手順の全体は[デプロイガイド §4c](../deploy/cloudrun/README.md#4c-optional-the-data-agent)にある。既定: 未設定(チームの Web UI は提案された SQL を見せるだけで、利用者が自分で走らせて結果を貼る) |
| `OCHAKAI_BIGQUERY_PROJECT` | Web UI(`ochakai ui` と serve-ui のどちらも)が提案された SQL を走らせるときに**課金するプロジェクト**(Google Cloud のプロジェクト ID)。設定すると、ページは課金プロジェクトの欄を出さず、全員がこのプロジェクトで実行する — 利用者に要るのは、このプロジェクトで BigQuery のジョブを作る権限(`roles/bigquery.jobUser`)と、読むデータへの読み取り権限だけで、プロジェクト ID を知っている必要は無い。未設定なら、ページが実行のたびに欄を出し、利用者が自分のプロジェクトを書く(ブラウザごとに覚える)。`stats` の `agent.bigquery_project` としてページに渡る。`OCHAKAI_AGENT` 無しでは起動を断り、プロジェクト ID として読めない綴りも起動エラーにする。既定: 未設定(利用者が書く) |
