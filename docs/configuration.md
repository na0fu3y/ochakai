...
| `OCHAKAI_AGENT` | このデプロイ自身の**データエージェント**を入れ、そのモデルを名指す(設計ドキュメント [0142](design/0142-ochakai-carries-a-data-agent-that-does-not-rule.md))。**既定は off で、未設定のあいだ ochakai は今日と一バイトも変わらない。** モデル id だけ(例 `gemini-2.5-flash`): メタデータサーバーから読んだこのデプロイの**プロジェクトとリージョン**で動かす — 埋め込みと同じく、テキストは動いているリージョンで処理される。**リージョンごとに答えるモデルは違う**: 2026-09 の asia-northeast1 では `gemini-2.5-flash` が答え、`gemini-3.x` 系は `global` にしか無い(設計ドキュメント 0142 §10)。後者を使うなら resource name で `locations/global` を名指す — テキストがどこで処理されるかを自分で選ぶことになる。**Vertex AI のモデル resource name**(`projects/<project>/locations/<location>/publishers/google/models/<model>`): そこで動かす。呼ぶのはサービスの身元で、API キーは読まない(`roles/aiplatform.user` が要る)。**名指したデプロイはエージェントを要求したことになり**、リージョンが読めない・モデルがそのリージョンで答えない・権限が無い、のどれでも起動を拒否する — 別のリージョンに黙って送ることはしない。エージェントは Web UI のレビュー画面と `POST /api/v1/agent` から使え、**呼び出した人として読み、書くのは新しい draft だけで(`process:ochakai via <呼び出した人>` として記録され、在る concept は書き換えない。`read-only` では書かない)、何も裁定しない**。数字が要る問いには SQL を**提案**するが、サーバーは SQL を実行しない — 走らせるのはそれを読んだ人自身で、手元の `ochakai ui` ならそのまま走り(プロキシが本人の gcloud の身元で、BigQuery の dry run で SELECT と分かったものだけ、上限 10 GiB)、チームの Web UI では `OCHAKAI_OAUTH_CLIENT_ID` があれば走る。`OCHAKAI_MODE=public` / `sandbox` とは組み合わせられない(誰も名乗らない呼び出しに運用者のモデルを開かない)。`off` のような綴りは、off が既定なので起動エラーにする。既定: 未設定(off) |
