...
| `OCHAKAI_AGENT` | このデプロイ自身の**データエージェント**を入れ、そのモデルを名指す(設計ドキュメント [0142](design/0142-ochakai-carries-a-data-agent-that-does-not-rule.md))。**既定は off で、未設定のあいだ ochakai は今日と一バイトも変わらない。** モデル id だけ(例 `gemini-2.5-flash`): メタデータサーバーから読んだこのデプロイの**プロジェクトとリージョン**で動かす — 埋め込みと同じく、テキストは動いているリージョンで処理される。**リージョンごとに答えるモデルは違う**: 2026-09 の asia-northeast1 では `gemini-2.5-flash` が答え、`gemini-3.x` 系は `global` にしか無い(設計ドキュメント 0142 §10)。後者を使うなら resource name で `locations/global` を名指す — テキストがどこで処理されるかを自分で選ぶことになる。**Vertex AI のモデル resource name**(`projects/<project>/locations/<location>/publishers/google/models/<model>`): そこで動かす。呼ぶのはサービスの身元で、API キーは読まない(`roles/aiplatform.user` が要る)。**名指したデプロイはエージェントを要求したことになり**、リージョンが読めない・モデルがそのリージョンで答えない・権限が無い、のどれでも起動を拒否する — 別のリージョンに黙って送ることはしない。エージェントは Web UI のレビュー画面と `POST /api/v1/agent` から使え、**呼び出した人として読み、ナレッジを書かず、何も裁定しない**。サーバーは SQL を実行しない — `OCHAKAI_OAUTH_CLIENT_ID` があればエージェントは SQL を**提案**し、走らせるのはそれを読んだ人自身である。`OCHAKAI_MODE=public` / `sandbox` とは組み合わせられない(誰も名乗らない呼び出しに運用者のモデルを開かない)。`off` のような綴りは、off が既定なので起動エラーにする。既定: 未設定(off) |
| `OCHAKAI_OAUTH_CLIENT_ID` | エージェントが**SQL を提案する**ようにし、Web UI がそれを**本人の身元で**実行できるようにする Google OAuth の Web クライアント ID(`<number>-<id>.apps.googleusercontent.com`、設計ドキュメント [0142](design/0142-ochakai-carries-a-data-agent-that-does-not-rule.md) §4)。**secret ではない** — 公開の識別子で、`stats` がページに渡し、ページが Google のサインインをポップアップで開いて `bigquery.readonly` のトークンを受け取り、BigQuery を直接呼ぶ。トークンはページのメモリにだけあり、サーバーには届かない。実行するのは、提案を読んだ人が「実行」を押したときだけで、一回の課金上限は 10 GiB。Google Cloud のコンソールで種類「ウェブアプリケーション」のクライアントを作り、**承認済みの JavaScript 生成元**に Web UI のオリジン(`ochakai ui` なら `http://127.0.0.1:8098`)、**承認済みのリダイレクト URI** に `<オリジン>/oauth.html` を登録する(ポートまで完全一致で照合されるので、`ochakai ui --port` で別のポートを使うならそれも登録する — `ochakai ui` は既定以外のポートで起動したとき、そう言う)。`OCHAKAI_AGENT` 無しでは起動を断る。クライアントの作り方を含む手順の全体は[デプロイガイド §4c](../deploy/cloudrun/README.md#4c-optional-the-data-agent)にある。既定: 未設定(エージェントは SQL を提案しない) |
| `OCHAKAI_BIGQUERY_PROJECT` | Web UI が提案された SQL を走らせるときに**課金するプロジェクト**(Google Cloud のプロジェクト ID)。設定すると、ページは課金プロジェクトの欄を出さず、全員がこのプロジェクトで実行する — 利用者に要るのは、このプロジェクトで BigQuery のジョブを作る権限(`roles/bigquery.jobUser`)と、読むデータへの読み取り権限だけで、プロジェクト ID を知っている必要は無い。未設定なら、ページが実行のたびに欄を出し、利用者が自分のプロジェクトを書く(ブラウザごとに覚える)。`stats` の `agent.bigquery_project` としてページに渡る。`OCHAKAI_OAUTH_CLIENT_ID` 無しでは起動を断り、プロジェクト ID として読めない綴りも起動エラーにする。既定: 未設定(利用者が書く) |
