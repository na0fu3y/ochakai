...
| `OCHAKAI_AGENT` | このデプロイ自身の**データエージェント**を入れ、そのモデルを名指す(設計ドキュメント [0142](design/0142-ochakai-carries-a-data-agent-that-does-not-rule.md))。**既定は off で、未設定のあいだ ochakai は今日と一バイトも変わらない。** モデル id だけ(例 `gemini-3.8-flash`): メタデータサーバーから読んだこのデプロイの**プロジェクトとリージョン**で動かす — 埋め込みと同じく、テキストは動いているリージョンで処理される。**Vertex AI のモデル resource name**(`projects/<project>/locations/<location>/publishers/google/models/<model>`): そこで動かす。呼ぶのはサービスの身元で、API キーは読まない(`roles/aiplatform.user` が要る)。**名指したデプロイはエージェントを要求したことになり**、リージョンが読めない・モデルがそのリージョンで答えない・権限が無い、のどれでも起動を拒否する — 別のリージョンに黙って送ることはしない。エージェントは Web UI のレビュー画面と `POST /api/v1/agent` から使え、**呼び出した人として読み、何も書かず、何も裁定しない**。SQL も実行しない。`OCHAKAI_MODE=public` / `sandbox` とは組み合わせられない(誰も名乗らない呼び出しに運用者のモデルを開かない)。`off` のような綴りは、off が既定なので起動エラーにする。既定: 未設定(off) |
