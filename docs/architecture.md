...
- **identity が言うのは誰かであって、何かではない。** 本人の
  ...
  MCP ではクライアント自身の名乗り)がソフトウェアの名前を
...
文書だけのハッシュであるため、concept を確認すること、利用を報告する
...
(設計ドキュメント [0146](design/0146-search-and-the-default-a-base-was-made-with.md) §4)。
...
rejection — レビューされて*受け入れられなかった*こと — はこのモデルの
中に**居場所を持たない**。それは concept が置かれる状態ではなく起きた
出来事であり、却下は理由を添えた削除である(設計ドキュメント
[0135](design/0135-a-rejection-is-a-deletion.md))。理由は削除を記録した
リビジョンに載り、OKF SPEC §9 の `log.md` が `**Reject**` の一行として
刷る — frontmatter のキーは一つも増えない。**否定を運べる段が OKF に
無い**からである: trust の三段は単調で、拡張キーに乗せればこのインスタンス
の観測が他所へ旅する主張になってしまう(0135 §2、
[0075](design/0075-the-bundle-is-the-address-space.md) §3.1)。エージェント
は同じことを再提案する前に log を読めるが、読ませて止めるものは無い —
却下された id はどの面からも書き直せる(0135 §3)。
...
ドキュメント [0142](design/0142-ochakai-carries-a-data-agent-that-does-not-rule.md) §8)。Redis も、
...
— 索引が読むだけで、検証もせず、専用のフィルタも持たない。ウェアハウスの
列が `revenue` で会議に出る語が `売上` のとき、その二つを結ぶ場所がこれで
ある(設計ドキュメント
...
既定で on になる**(設計ドキュメント
[0146](design/0146-search-and-the-default-a-base-was-made-with.md) §1。何がそれを決めるか、
...
設計ドキュメント [0142](design/0142-ochakai-carries-a-data-agent-that-does-not-rule.md) が賭けている
のは、エージェントが広さを、人間が判断を供給するということである:
エージェントは学んだことを `draft` として書き、人間はそれを
確認するか、deprecate するか、理由を付けて reject する。provenance
とリビジョンが、それを後から review 可能にする。この四つのフィード
