...
**Golden queries are stored as `Attested Computation` concepts.** ochakai
...
answers. If
your base predates this, concepts typed `Golden Query` keep working as free
...
A hit names a concept rather than handing it over: the ranking is what a
...
unverified concepts last. "Verified queries not re-checked in 90 days" is
...
Execute each concept's `# Computation` fence against the warehouse
...
- **It ran clean**: record the re-verification —
  ...
  are appended to the concept's verification ledger, so the concept moves to
...
- **Failure or warning**: create a draft `Insight` (`kind: caveat`)
  against the affected concept, or propose `status: deprecated` with a
  `status_note` giving the reason. A human decides, looking at the
  provenance. Once the concept is established to be wrong, it becomes
  `ochakai reject <id> --note "…"`. Rulings are made from
  the human surfaces (web UI / CLI): if an agent is running your canary,
  overwriting a verified concept and changing its status are both refused
...
when it was last used. **A verified concept nobody has used in a long time**
is a reason to lower its canary priority — or to consider deprecating it.
**A verified concept that has collected failures** is the reverse: a reason
