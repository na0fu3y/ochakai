...
`created_by` は .md の中にある — 誰かが `verify` を打てば、
次の export で `verified:` に一行増え、その行は検証した人の identity を
持つ。**PR でそれを書き換えても意味は無い**: 取り込みは読み戻さず、
`received:` の下へ主張として落とすだけである(0075 §3.1)。リビジョン
履歴も同じ扱いで、各段の `log.md` として出て行くが読み戻されない —
却下の理由がそこに載るのはこのためである(設計ドキュメント
[0135](../design/0135-a-rejection-is-a-deletion.md) §2)。バンドルが
運ばないのは利用回数と結果報告のほうで、レビューが裁くのは知識の側
だけでよい。
