...
を書く。なぜ provenance がバンドルに乗らないかという決定そのものは
...
**台帳は Git に出ない。** 検証・却下・利用回数・リビジョン履歴は、この
インスタンスが観測したものであってバンドルが運ぶものではない。だから
PR の diff に出るのは**知識が何と言っているか**だけである。これは
狭さではなく、レビューが読むべきものだけが出るということである。
...
Verify である。Git を経路にするチームは、この二つを**別々に置いたまま
