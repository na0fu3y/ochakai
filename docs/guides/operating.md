...
編集できる」。拒否規則は無く、**どの規則も付与しなかったものは読めない**。
照合はセグメント境界なので `sales` は `sales/orders` に当たり
`sales-legacy/orders` には当たらない。principal は台帳と同じ綴り
(`human:` / `process:`、`*` は全認証済み呼び出し元)で、生のメール
アドレスは受け付けない。

**「編集は数人、閲覧は全員」は、根への付与で書ける。** 空 prefix は
バンドル全体で、行数は社員数ではなく編集者の数で決まる。根を読めること
は、管理者であることではない。

```json
{"rules": [
  {"prefix": "", "principal": "*",                          "may_write": false},
  {"prefix": "", "principal": "human:tanaka@example.co.jp", "may_write": true}
]}
```
...
その相手の保存を跨いで残るので、上の手順をそのまま渡してよい。
...
**バンドル全体への `may_admin` は置けない** — それは `OCHAKAI_ADMINS` の
仕事である。そして**渡した相手より浅い規則は、その相手には見えないし
消せない**: `prefix: ""` に `*` の読み取りを与えてあれば `teams/growth`
にも効き、`teams/growth` の管理者はそれを知らない。預ける前に、上から
効いている行がないか読み返すこと。
...
1. **バンドル全体を取る操作は管理者のものになる** — `ochakai export`
   ...
   **該当なしの検索(miss)は差し止められる**: 何も返さなかった検索には
   ...
   しまう。`misses withheld` と出るのはそれで、ゼロではない。**根を付与
   された呼び出し元は範囲を持たない** — 数はバンドル全体のもので、`scope`
   行も出ず、miss も出る。cron の `stats --exit-code` を回しているなら、
   その呼び出し元のキューだけを見ることになる — インスタンス全体を
   見張りたいなら、根の読み取りを一行与えるか、管理者にする。
2. **スコープ外は 404 で、403 ではない。** 「そこに何かあるか」への
   答えが境界そのものである。書けるが読めない、は起こらない。
3. **本文の散文は塞げない。** 読める concept の本文が隠した id を名指し
   ていれば、その id は本文の中に現れる。約束しているのは一覧・検索・
   取得・エクスポート・index から出てこないことである。秘匿する対象を、
   公開側の concept が引用していないかはキュレーションの問題として残る。

管理者を空にしたまま行が残っているデプロイは**起動しない**。抜けるには
`OCHAKAI_ADMINS` を設定して再デプロイする(起動時のエラーがそう言う)。

**web UI にも同じものがある。** 管理者として届いた呼び出し元にだけ
「アクセス」タブが出て、上の表を表示する。編集は**行の表**で、
ディレクトリ・対象・できること(読取のみ / 読取・書込 / 読取・書込・付与)
を選び、行を足したり消したりする — 送られるのはこの CLI と同じ文書
一枚で、保存は表の全部で置き換える。その文書は「JSON として扱う」の
中にあり、git に入れるものはそこからコピーし、そこに貼ったものは
「行に取り込む」で表に入る。ただし `serve-ui`(下の「チーム向け web
UI」)は呼び出し元をサービスの SA 一つに畳むので、**IAP を前置して
いなければ、その URL に届く全員がその SA としてポリシーを編集できる**。
UI は読み返しと、急ぎの一行のためにある。
...
デプロイガイドのすべては、Cloud Run の IAM に身元を任せて誰かが書き込める
あらゆるデプロイに対して「`allUsers` は決してだめ」と言っており、それは
今も正しい(トークンを自分で検証する Claude のコネクタのデプロイは別で、
デプロイガイド §5e にある)。デモは
...
[examples/demo](../../examples/demo) は読ませるために作られた 36 個の
...
その 36 個の concept は `human:you@your-org.example` として記録される —
このベースが受け取る最後の provenance であり、切り替えの後に訪問者が
見る唯一の `created_by` である。(この手順はローカルサーバーに対して
リハーサル済みである: 書き込み可能な状態で import — 36 created、0
...
**ochakai は呼び出し元ごとの回数制限を持たない。** 誰が届くかを決める
のは Cloud Run IAM で、ochakai の中には一行も無い(C2)。どれだけの頻度で
届くかも同じ層の仕事であり、同じ理由で中には置かない。デモの天井は上の
`--max-instances=1` で、インスタンスが飽和すれば Cloud Run 自身が 429 を
返す — 課金には天井が付いているが、一つのクライアントが容量を使い切れば
他の訪問者も同じ 429 を見る。訪問者間の公平性が要るなら、外部 HTTPS
ロードバランサの後ろに置いて Cloud Armor のレート制限ポリシーを張る。
本物のクライアント IP を UI と API の両方の前で見られるのはそこだけで
ある — API が直接見る接続元は UI サービスの一つの IP であり、public
姿勢は `X-Forwarded-For` を信じる根拠を持たない。中に持てば search と
bundle という凍結された操作に 429 を足すことになり(0082 §2)、数字を
変えるのはリリースになる。ロードバランサの固定費が月 $10 のデモに
乗るのは承知の上の代金で、公平性が要るまでは払わなくてよい。
...
**いま何が動いているかは、デプロイ自身が言う。** `GET /api/v1/stats` が
`version` を返し、`ochakai whoami` の `version:` 行がそれを印字する。
`ochakai version` が印字するのは**手元のバイナリ**であって、アップグレード
の最中に知りたいのはそちらではない。この行を持たない古いサーバーに当てた
ときは、推測せずに黙る — 空欄ではなく、行そのものが出ない。

**web UI は二つ言う。ここで動いているものが二つだからである。** `serve`
は UI を配信しないので、ページを配ったビルドと `/api/v1` に答えている
ビルドは常に別のプロセスであり、下の `webui_image_tag` はその二つを一度の
apply で意図的にずらすためだけに存在する。見出しの右に出るのは、
**一致している間は一つの番号**（それが両方の答えである）、**食い違った
瞬間に両方**が `UI …` / `API …` と名前つきで並ぶ。並んだらそれが下の罠の
一つ目そのものであり、他のどこにも音を立てない状態である。片側しか名乗ら
ないとき（version を持たない古いサーバー、あるいは二つのコマンド以外が
配ったページ）は、名乗った側だけがラベル付きで出る — 半分を全体として
読ませないためである。
