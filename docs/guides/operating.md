...
`version` を返し、web UI はそれを見出しの右、製品名の隣に描く —
`ochakai whoami` の `version:` 行も同じ答えである。`ochakai version` が
印字するのは**手元のバイナリ**であって、アップグレードの最中に知りたいのは
そちらではない。この行を持たない古いサーバーに当てたときは、どちらの面も
推測せずに黙る — 空欄ではなく、行そのものが出ない。
