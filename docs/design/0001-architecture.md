...
Status: Accepted(2026-07-14 の議論で主要論点を決定)。§3 の共通エンベロープの
...
### 2026-07-17 追記: エージェントメモリレイヤ(mem0 / Zep / Letta)との対比

「エージェントが覚えてくれる」という体験の面で、ユーザーからはメモリレイヤと
同じ棚に見える。実際は半分しか競合していないが、比較質問には答えられる必要が
あるため記録する。ワンライナー: **memory layers remember what happened;
ochakai curates what's true.**

| 事例 | 概要 | ochakai との対比 |
|---|---|---|
| mem0 | 会話から LLM が記憶を自動抽出し、次の会話に自動注入。スコープは user_id / agent_id 単位。OSS 版は抽出用の LLM API キーが必須、Platform は SaaS。26k+ stars で「エージェントにメモリを 3 行で」の代名詞 | 記憶は無監査(誰が書いたか・誰が確認したかの概念がない)。誤った記憶が静かに残り続ける。チーム共有・検証ライフサイクル・provenance が ochakai の差分。逆に「エージェントの判断なしで自動保存・自動想起」は mem0 の強みで、ochakai は hooks + `get_context` で同等の体験を LLM なしで再現する(examples/claude-code) |
| Zep | 会話履歴 + ビジネスデータを temporal knowledge graph に統合(Graphiti)。事実の有効期間を追跡 | 時系列グラフによる「いつ正しかったか」は ochakai の revision + verified_at と部分的に同型。ただし自動構築・無監査で、人間のレビューゲートがない |
| Letta (MemGPT) | エージェント自身が自分のメモリを編集する(self-editing memory)。エージェント OS 志向 | メモリはエージェント個体に属する。チームの知識ベースではなく、検証・却下の概念もない |
| A-MEM / OpenMemory ほか | Zettelkasten 型の自動リンク生成など研究系が続々。共通するのは「LLM が記憶を自動管理」 | 同上。自動化が本体で、キュレーションが本体の ochakai とは設計の重心が逆 |

構造的な違い(ochakai だけが持つもの):

1. **検証ライフサイクルと「No の記憶」**。draft → verified / rejected(理由付き)。
   メモリレイヤには「人間がレビューして却下し、その却下理由が再提案を止める」
   仕組みがない。データ分析では誤った記憶(間違った SQL、古いベースライン)が
   そのまま誤った意思決定になるため、無監査の自動記憶は品質の天井が低い。
2. **チーム共有・クライアント非依存**。メモリレイヤの記憶はユーザー/エージェント
   個体スコープ。ochakai のナレッジは組織で共同所有され、Claude Code・hosted
   MCP・CI のどこからでも同じものが引ける。
3. **LLM ゼロ**。抽出も注入も決定論的。抽出コスト・抽出誤りがない。

併用ストーリー: ユーザーの好み・会話文脈は mem0/Zep へ、データの検証済み知識は
ochakai へ。競合質問への一番強い答えは「別物で、補完」。

体験面の教訓も取り込む: メモリレイヤの本質的な強さは記憶の質ではなく
**エージェントの判断が要らない**こと(自動保存・自動想起)。ochakai はこれを
`get_context`(一発でコンテキストパックを返す)+ Claude Code hooks(recall /
write-back)で実装した。CLAUDE.md は「読まれるかもしれない指示」、フックは
「必ず実行される仕組み」であり、習慣の摩耗に強いのは後者。
...
**2026-07-17 追記(唯一の例外)**: `get_context` / `GET /api/v1/context` /
`ochakai context` は例外とする。これらはナレッジ実体の CRUD ではなく、
「ナレッジから組み立てられ、エージェントに届けられる側」— まさに上記の
context の定義そのもの — を返す唯一の面であり、ここに `knowledge` を使う方が
かえって用語の使い分けを壊す。実体を操作するツール・パス・型は引き続き
`knowledge` のみを使う。
...
§9.2 の利用イベントは「使われた」ことしか記録せず、「役に立ったか」が
還流しない — 書き戻しループの最後の一辺。エージェントが golden query を
実行して間違った数字を得ても、次のエージェントは同じ verified エントリを
同じ信頼度で引いてしまう。

- **イベント**: `worked`(結果が正しかった)/ `failed`(誤り・使えなかった)。
  受動的な §9.2 のイベントと違い、MCP ツール `report_outcome`
  (`{target, outcome, note?}`)による**能動的な報告**。actor と自由記述の
  note(何を実行し何が起きたか、最大 2000 バイト)を `knowledge_event` に
  記録し、`/usage` の集計に worked / failed を出す。報告の失敗は呼び出し元に
  返す(read の副作用である利用記録と違い、報告は意図した書き込み)。
- **REST**: 新しい API 面は作らず、usage の正規パスに書き込み半分を足す:
  `POST /api/v1/usage/{type}/{id}`。CLI は `ochakai report`。
- **語彙は 2 値**: `misleading`(結果は出たが解釈を誤らせた)は note で
  表現できるため保留。必要が定量的に見えたら追加を再検討。
- **重複報告は dedupe しない**: actor 付きの生イベントが残るため、集計の
  ノイズが問題になってから対処(search_hit の重複計上と同じ扱い)。
- **ランキング反映は引き続き保留**(§9.5 の 2 と同じ理由)。failed が
  積んだ verified エントリは、まず Web UI の usage 表示とカナリア運用の
  再検証対象として人間/エージェントが拾う。
