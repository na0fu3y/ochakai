...
| [0027 Delegated end-user provenance](0027-delegated-provenance.md) | Accepted; composition reused by 0052 for producer; header respelled without `X-` by 0064 | A caller listed in `OCHAKAI_DELEGATING_CALLERS` may name an end user via `Ochakai-On-Behalf-Of`; both identities are recorded (`human:x via agent:sa`), and an unlisted caller's header is a 403 rather than a silent downgrade. Lets an application embedding ochakai attribute writes to its real users instead of one shared service account. |
...
| [0057 Concept is the word a reader meets too](0057-concept-is-the-word-a-reader-meets.md) | Accepted; **breaking (wording only)**; the current record for the unit of knowledge's name (carries 0054) | Renames the five MCP tools carrying `knowledge` to OKF's word (`search_concepts`, `get_concept`, `put_concept`, `delete_concept`, `get_concept_usage`; tool count stays 8) and completes that by renaming every word a reader meets — README, docs, examples, CLI help/output, web UI labels, MCP/OpenAPI descriptions, error text — from `entry` to `concept`; identifiers (the `entries` JSON key, Go type names, DB columns, the Japanese records) are untouched. Nothing on the wire moves; only a script matching on tool names or help/error text is affected. |
