...
| サーフェスの配分 | [0145](0145-four-faces-and-an-answer-that-shows-its-result.md) が現行 — 各面の役割と載せないもの、MCP の 6 本と stateless な転送、読みが search → get で単読が `linked_from` を運ぶこと、**デプロイ自身のエージェントが REST 一本の後ろにいること**、そして**答えが自分の結果を表と(形が許せば)グラフで見せること**(0143 を節の番号ごと置き換えた)。足す規則と降ろす規則は [0068](0068-how-a-face-is-added-and-removed.md)。一覧と検索を MCP でも二本に分けることは [0096](0096-a-listing-is-not-a-search-here-either.md)。**ツールの答えが一通で返ることと、予算がスキーマを両側とも数えることは [0103](0103-the-tool-result-travels-once.md)**。CLI がファイルを名指す綴りは [0140](0140-one-address-reads-as-well-as-writes.md)。CLI の行の第一列は [0110](0110-the-first-column-is-the-key-you-asked-for.md)、その太字と dim は [0111](0111-weight-for-the-eye-and-only-for-an-eye.md)。**Google Chat で問われたら、別サービスの橋 `serve-chat` が本人の代わりに REST の一本に問うことは [0150](0150-google-chat-asks-through-a-bridge.md)**(secret なし、Chat では SQL を走らせない、Slack の橋は作らない) |
...
- [0085 空のベースと、それを埋めるもの](0085-the-empty-base-and-what-fills-it.md) —
  **Superseded by 0148**。`ochakai seed`(接続しない CLI、draft、空の
  description)を決め、Web UI には載せなかった前身。
- [0148 空のベースは、ページからも本人として埋まる](0148-the-empty-base-fills-from-the-page-too.md) —
  **Accepted**。0085 を置き換える。`seed` の決定はそのまま再掲し、**Web UI が
  データセットのスキーマを本人の身元で読んで同じ draft を作る**。0085 が
  UI を断った理由(読むなら資格情報がサーバーに来る)は、0142 §4 がページに
  本人のトークンで BigQuery を読む手段を入れたことで外れた。書き込みは
  既存の `PUT` を `If-None-Match: *` で一件ずつ — 既にある住所は、誰かが
  書き足した説明ごと残す。50,000 行を超える一覧は一部ではなく断る。
  `OCHAKAI_OAUTH_CLIENT_ID` / `OCHAKAI_BIGQUERY_PROJECT` はエージェント
  無しで立つ。投影は Go と JS の二か所にあり、一つのゴールデンファイルが
  縛る(サーバーの投影操作と WebAssembly は採らなかった)。surface.md の
  数はどれも動かない。
...
- [0147 検索が何を融合し、既定の埋め込みはベースが生まれたときに決まる](0147-search-and-the-default-a-base-was-made-with.md)
  ...
  `gemini-embedding-2`(`global`)にする。** 前からあるベースは
  `gemini-embedding-001` を動いているリージョンで使い続け、保存済みの
  ベクトルもテキストの行き先も変わらない。見分けるのはベースで、起動時に
  `schema_migrations` が空だったかを一度だけ読み、`base_birth` の一行に
  残す(ベクトルの有無で見分ける案は、字句だけで動いている既存デプロイを
  黙って `global` へ送り始めるので退けた)。2 は画像と PDF を検索に載せ、
  入力窓が四倍で、`global`/`us`/`eu` にしか居ない — 既定を移すことは
  新しいベースのテキストが `global` で処理されることを製品が選ぶことで、
  本書はそれを明記して選ぶ。所在地を固定したい運用者と、既存のベースを
  移したい運用者は、どちらも resource name で言う(変数は増えない)。
  幅は 768 のまま(広げれば既に 2 を名指したデプロイのテーブルが空に
...
- [0150 Google Chat は橋を通して問う](0150-google-chat-asks-through-a-bridge.md)
  — **Accepted**。ROADMAP 第四段の最初の一つ。`ochakai serve-chat`(`serve`
  と同じイメージの別 Cloud Run サービス)が Chat のイベントを受け、
  `POST /api/v1/agent` に本人の代わりに問い(`Ochakai-On-Behalf-Of`、
  `OCHAKAI_DELEGATING_CALLERS`)、答えを同期の HTTP 応答で返す — エージェントに
  第二の入口は作らない(0145 §7)。secret なし: Chat の呼び出しは Google が
  署名した ID トークン(audience はエンドポイント URL、email は Chat か
  Workspace アドオンのサービスアカウント)を橋が確かめ、`serve` には橋の
  サービスアカウントで届き、Chat API は呼ばない。インタラクションイベントと
  アドオンのイベントの両方を読む。Chat では SQL を走らせず(本人のトークンが
  無い)、提案を見せて Web UI へ誘う。25 秒で打ち切り、スレッドの前の発言は
  読まない。どの数も動かない。却下した案: `serve` が直接受ける、非同期の
  返事、Chat のユーザー認可、Slack の橋(署名の secret とボットのトークン)、
  プロジェクト番号の audience。
...
- [0127 アーカイブは、自分がどの部分かを言う](0127-an-archive-says-which-part-it-is.md) — **Superseded by 0134**。
- [0134 アーカイブは、自分がどの部分かを言う](0134-an-archive-says-which-part-it-is.md)
  — **Accepted**。[0127](0127-an-archive-says-which-part-it-is.md) を畳んで
  吸収する(0128 §2.2 — 差分ではなく領域を丸ごと)。0109 §3 の
  「export は管理者のもの」を改訂する — **全体は管理者のまま、subtree は
  読める者のもの**。**測ったら危険の形が違った**: `ochakai import` は加算的で
  削除しないので「戻したときに残りが消える」は起きず、本当の危険は**部分と
  全体が同じ形で届くこと**だった — subtree アーカイブも根の `index.md` を持ち、
  名前も同じ `ochakai-okf.tar.gz` で、**どちらかを言うものが一つも無い**。
  つまり 0109 §3 が恐れた事態は断っている間もすでに作れており、断ることは
  それを一度も防いでいなかった。決定は、読む人が出会う二箇所に書くこと —
  根の index の**見出しと本文の一文**、そしてファイル名
  `ochakai-okf-<subtree>.tar.gz`(**復旧する人は展開する前にファイル名を
  打つ**)。**0127 から動いたのはここだけである**: 0127 は機械向けに
  frontmatter の `bundle_scope` を置いたが、SPEC §8 は予約ファイルの
  frontmatter に `okf_version` 以外の置き場を持たず(§11 の適合条件 3 が
  §8 に縛る)、`--prefix` のアーカイブは **OKF v0.2 に適合していなかった** —
  未知の鍵を許す §4.1 は concept 文書の規則で、ここには届かない。しかも
  同じことを見出しと本文とファイル名が既に言っており、見出しは §8 が形を
  決めている当のものなので、機械の読む口も残る。深い階層の index も全体の
  アーカイブも一字も変わらない。見分けが付く以上、断ることが買うものは
  無くなる — subtree の中身は呼び出し元が concept を一つずつ取れば既に
  得られるもので、断っていたのは知識ではなく便宜だった。**根は 403、
  読めない subtree は 404**。これで
  [0119](0119-an-operated-fleet-is-deployments-or-directories.md) §4 が
  代金に数えた「出口(C1)に運用者が要る」が返る。面はどれも動かない
  (`--prefix` は既にある綴り)。却下: `bundle_scope` を適合しないまま残す、
  本文に機械向けの新しい綴りを発明する、import に読ませて警告(予約ファイルを
