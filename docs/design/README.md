...
- [0043 文書を真とする](0043-document-first.md) — **Accepted**(実装済み・
  未リリース。破壊的変更は 0.15.0 の次のリリースで出る。§3.11 の Web UI の
  フォーム糖衣は 0044 が撤回。§3.2 のインポート時の検証の扱いは 0045 が
  改訂 — 台帳そのものの姿はここが現行)。
...
- [0045 取り込んだ検証と、この場で下した検証を分ける](0045-adopted-verifications.md)
  — **Accepted**(実装済み・未リリース)。0043 §3.2 の「バンドルの
  `verified` キーは存在すれば検証 1 行を起こす」1 点を改訂し、採用を
  `import --adopt-verified` という明示の行為にしたうえで、検証行に
  `source` を持たせる — 空が「この場でレビューした」、非空が
  「どこから採ったか」。`by` は変わらず取り込んだ actor である
  (payload は名乗れない、0009 §3.2 は維持)。`source` は検証者自身の
  申告であって信頼の境界ではなく(§3.3)、エクスポートには出ていかない
  (§3.6)。台帳そのものの姿は 0043 が現行。
