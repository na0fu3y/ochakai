...
「`/` 不可」も改訂する。
**§11 の「凍結を破ってよい唯一の理由はセキュリティ上の欠陥である」は
[0082](0082-what-the-freeze-holds-still.md) が改訂した** — 応答専用
スキーマへのプロパティ追加は、そもそも凍結が止めているものではない。
**§11 の「どんなジェネレータもこの一つの住所については動く
クライアントを生成しない」は
[0098](0098-the-generator-works-what-was-missing-was-the-declaration.md)
が取り下げた**(2026-08-10。欠けていたのは `put` と `delete` の
パスパラメータの宣言だった)。
**§11 の「凍結を破ってよい唯一の理由はセキュリティ上の欠陥である」に
[0100](0100-md-is-how-a-concept-is-spelled.md) が二つ目を足した** ——
書き込み面が OKF SPEC §11 の適合条項に反する出力を作れてしまう場合。
同記録は §16(本文が種類を決める)と §20.4 の隙間の記述も畳んでいる。
**三つ目は [0102](0102-one-history-in-one-spelling.md) §3 が足した** ——
規格が綴りを定めているものの二つ目の綴りを畳むこと(三条件を全部満たす
場合に限る)。同記録は §14.1 の「`?history` の `limit` は対象で違う」も
撤回する —— markdown の履歴が無くなり、概念の答えは一つになった。
**凍結の範囲は [0107](0107-the-freeze-holds-the-okf-core.md) が改訂
した**(2026-08-16)—— 凍るのは OKF コア(bundle の往復と search の
4 操作)だけになり、残りの `/api/v1` は 0.x の不安定な面に戻る。
**§2 の「宣言していないボディキーは 400 で名指す」を、書かれたとおりに
効かせたのは [0122](0122-a-body-names-each-field-once.md) である**
(2026-08-23)—— 照合が大小を区別せず `RULING` が通っていたことと、
同じ鍵の重複が黙って最後の値を採っていたことの二つ。後者は §20.2 の
答えを本文に当てたものである。
「REST は止まった」という本書の宣言のうち、コアの外への半分は撤回で
ある。未知キーの 400・`/api/v2` を作らないこと・§11 の破ってよい理由は
コアに対してそのまま残る。
他の決定は動いていない。
Date: 2026-08-02
...
**追記(§23 の監査)**: 本節が「書き込む値そのもの」と呼んだ
`PutAttachment` / `DeleteAttachment` には、0075 §5 がファイルを自分の
path のオブジェクトにして以来**呼び出し元が無い**。今日のファイルの行は
`PutFile` / `DeleteFile` がその path 自身の `create` / `delete` として
書き、`add_file` / `remove_file` はマイグレーション 0035 が書き換えた
過去の行 — ファイルの出来事が概念の改訂だった時代のもの — にだけ載る。
改名の決定はそのまま立つ(過去の行が `attach` と言い続けない)が、契約の
Change / Revision の散文は二語が歴史の綴りであることを言い、log.md の
例は現行の綴りを見せる。
...
- `status` / `trust` に語彙圏外の値を送り、空ヒットを当てにしていたら、
  **400 として扱う**(§23.3)。
- `fm.id` / `fm.status_note` を引いていたら、**外す**(§23.5)。id の
  問いは住所と `prefix=` が答える。
- `POST /api/v1/reembed` に本文を送っていたら、**外す**(§23.4)。
- 負の `budget` を「上限なし」として送っていたら、**0 か省略にする**
  (§23.4)。
- dry run の応答の `observed.generated.by`・`content_hash`・時刻を
  「空になる」前提で読んでいたら、**実書き込みと同じ値**が入る(§23.1)。
  ファイル上書き(200)の `created_by` / `created_at` は**行の作成者**に
  なる(§23.2)。
...
§§15-22 の後にもう一周、契約と実装と OKF SPEC を突き合わせた。見つかった
のは §2 と同じ族の沈黙が三つ、dry run だけが運ぶ嘘が一つ、SPEC に無い
キーを「OKF の」と呼んでいた語彙が一つ、そして「required と言えるのは
今日まで」の一群である。

### 23.1 dry run は書き込みが刻む値を刻む

`svc.Plan` は update の分岐で `UpdatedBy`・`ContentChangedAt`・
`UpdatedAt`・`ContentHash` を埋めずに返していた。wire に出るのは
`observed.generated.by = {"kind":"","name":""}` — **契約自身の閉じた
enum `[human, process]` に違反する値** — と、西暦 1 年の時刻、空の
content_hash である。create の計画も時刻とハッシュを欠き、ファイルの
計画は `created_by` / `created_at` を欠いた。**決定: 計画は Update と
同じ規則・同じ時計で埋める。** ハッシュは決定的なので、計画の
content_hash と実書き込みの ETag が一致することを統合テストが読む。

### 23.2 ファイル上書きの応答は行の値を言う

`PutFile` は上書き(200)でも `created_by: いまの呼び手`・`created_at:
いま` を返していた — 行は動かないので(`created_at` は ON CONFLICT の
SET に無い)、直後の GET と食い違う。**決定: RETURNING で行の値を読んで
返す。** あわせて、ファイル行は kind / name しか持たないので、書き込み
直後の応答も via / producer を運ばない — GET が言える以上のことを
言わない。

### 23.3 status / trust の圏外は 400

閉じた語彙の `status` / `trust` に圏外の値(`Draft`・`verified` など)が
来ると、200 の空ヒットが返っていた — 「その語は無い」が「知識ベースは
空だ」と読める、§2 と同じ形の沈黙である。**決定: `checkedFilter` で 400
で名指す。** どの面もこの一点を通るので、REST・MCP・CLI が同時に閉じる。

### 23.4 reembed の本文と、負の budget

`POST /api/v1/reembed` は本文を一切読まず、`{"limit": 10}` を本文に
入れた呼び出しは黙って既定の 200 件を走らせていた — §16 が受け入れた
「バリデータが見ない」の隣にある、実行時の沈黙そのもの。**非空の本文は
400 にする。** `budget` は負値が「上限なし」に落ちていた — 符号の
間違いが全部を貰う。**負値は 400**、上限なしの綴りは 0 か省略だけで
ある(§10 の limit / days と同じ形)。

### 23.5 fm. の語彙は OKF の定義キーだけになる

`fm.` の引ける集合に `id` と `status_note` が載っていた。どちらも SPEC
v0.2 に無い — トップレベルの `id` は定義されず(concept の id はパスで
ある。§5.1 の `sources[].id` は別のキー)、`status_note` はどこにも
現れない。ochakai 自身が producer として書く拡張キーであり、拒否の
メッセージ自身が「語彙は OKF のもの」と言う以上、自分のキーだけ引ける
のはその理由を掘り崩す。**決定: 二つを読み出しから外す(400)。**
0074 §4.2 の導出は「EnvelopeKeys のうち OKF が定義するもの」に狭まる。
envelope としては残る — 保存と往復は変わらない。凍結後に外すことは
永久にできず、足すことはいつでもできる(§2)ので、迷いは小さい方に倒す。

### 23.6 常に在るものは required と言う、最後の日

エラーの封筒 `{"error"}`・search の `hits`・context の `hits` /
`concepts`・`Usage` の四カウンタ・`File` の七フィールド・`Stats` の
全域・`Actor` の kind / name — サーバが常に書くのに、契約は required と
言っていなかった。**required の追加は指紋の行を動かすので、凍結後は
二度とできない。** 検証する生成クライアントが全フィールド optional の
型を永久に持つ前に、実装のタグと突き合わせて宣言した — 指紋の diff は
required=false → true の 97 行だけである。

### 23.7 散文が実装に追いつく

- PUT の記述にあった「`verified` キーの有無が `stable` の写像を決める」
  は存在しない規則だった(0043 / 0075 §4.1 が廃止済みで、0075 §3.1 も
  そう言っていない)。消した。
- `Summary.updated_at` の「generated.at と同じ瞬間」は §17 の後では嘘に
  なっていた — updated_at は行の時刻、generated.at は内容の時刻。
- log.md の例の `add_file`(§8 の追記のとおり現行は `create`)、バンドル
  GET の anyOf の「four」(実際は五つ)、ファイル PUT の 409 に概念パス
  衝突が無いこと、review の 400 の例文、Type の pattern がコードより
  緩いこと(制御文字とバイト数の但し書きを散文に)、アーカイブ内の
  log.md が 1000 行で切れること — いずれも記述を実装に合わせた。
- `Ochakai-Read-Only` は「すべての応答」と約束しながら、httpauth が書く
  400 / 403 には立っていなかった。main が認証 middleware の外側にも
  同じラッパを掛ける(§14 と同じ「実装が契約に追いつく」形)。
