...
_ochakai() {
  ...
  case $words[2] in
    ...
    export)
      ...
      ;;
    import)
      _arguments '--dry-run[parse and list, write nothing]' '--diff[say what would change, write nothing]' '--strict[fail on any note or skip]' '--adopt-verified[confirm the entries whose documents carry a verified key]' '--url[server URL]:url:' '1:bundle:_files'
      ;;
    use)
      local -a servers
...
_ochakai() {
  ...
  case $cmd in
    ...
    import)        opts="--dry-run --diff --strict --adopt-verified --url" ;;
...
complete -c ochakai -n '__fish_seen_subcommand_from import' -l adopt-verified -d 'confirm the entries whose documents carry a verified key'
