...
func TestClientCommandsCoverDesignDoc(t *testing.T) {
	for _, name := range []string{"search", "context", "get", "create", "update", "delete", "attach", "detach", "usage", "report", "compile", "export", "import", "import-ossie", "use", "whoami", "ui", "completion"} {
		if _, ok := clientCommands[name]; !ok {
			t.Errorf("missing client command %q", name)
		}
	}
	if len(clientCommands) != 18 {
