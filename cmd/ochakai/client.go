...
func cmdUsage(ctx context.Context, args []string) error {
	...
	// The two the feeds rank on, named with their window: the lifetime
	// counts above say what a concept was ever worth, these say whether
...
func cmdStats(ctx context.Context, args []string) error {
	...
	fs.Var(&prefixes, "prefix", "measure only concepts under this `path`, e.g. teams/growth — matched on segment boundaries (repeatable, OR-ed). The unanswered questions are not scoped: one no concept's words matched has no concept to belong to")
