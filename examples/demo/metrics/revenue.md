...
で束ねて合計したものである。式は `SUM(sale_price)` だけで、この数字に
ついての議論は、どの行が入るかについての議論になる。
...
この数字を社内で指す語が「売上」で、この concept が定義しているのは
それである。倉庫の側では `revenue` という列名で出てくるので、両方の
名前で引けるようにしてある。「トップライン」も同じものを指す。GMV は
この指標ではない。
