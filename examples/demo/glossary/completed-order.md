...
両方にある。このバンドルでは、常に明細側を読む。明細ごとに状態を持つ
ので、3点のうち1点だけ返った一部返品も、明細1行の `Returned` として
特別扱いなしに数えられる。注文側だけを読む集計は、
その1行を見落とす。
...
売上が今日見ると減っていたら、たいていこれが起きている。この扱いは
[売上計上ポリシー](/policies/revenue-recognition.md)が決めていて、締めた
数字の固定には receipt を使う。
