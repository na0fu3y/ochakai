...
2行になる。行数がそのまま個数なので、`SUM(quantity * price)` のような
式は書けない。1行が在庫の個体1つ(`inventory_item_id`)に対応している
ためである。
...
なるだけで正しく数えられる。集計を常に明細側で行うというこのバンドルの
規則は、ここから来ている。注文側だけを読む集計は、その1行を見落とす。
