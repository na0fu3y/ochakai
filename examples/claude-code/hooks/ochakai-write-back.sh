...
jq -n --arg outcome "$outcome_reason" '{
  decision: "block",
  reason: ($outcome + "Before finishing: this session did data work. If a query you wrote proved correct and reusable, or you learned how to read a metric (a baseline, a seasonality, a caveat), write it back to ochakai — search first to avoid re-proposing what is already there, then `ochakai put <path>` (type \"Attested Computation\" with runtime, the SQL in a # Computation fence, and a top-level question key; or type \"Insight\"). Link it to the concept it came from — an ordinary markdown link to that concept'"'"'s path in the body, [revenue](/metrics/revenue.md) — because that link is the relationship, and without it nobody reading that concept meets what you wrote. If nothing durable was learned, finish now without creating anything — do not invent knowledge.")
}'
